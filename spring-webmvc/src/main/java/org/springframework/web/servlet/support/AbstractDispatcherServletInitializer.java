...
public abstract class AbstractDispatcherServletInitializer extends AbstractContextLoaderInitializer {
	...
	/**
	 * 在 Spring MVC 中我们其实是可以创建出多个 DispatcherServlet 的（只要创建多个继承自 AbstractAnnotationConfigDispatcherServletInitializer 的类即可），
	 * 而每个 DispatcherServlet 有自己的应用上下文（WebApplicationContext），这个应用上下文只针对这个 DispatcherServlet 有用。这
	 * 也就是 getServletConfigClasses 的作用，获取这个 DispatcherServlet 的应用上下文的配置类。
	 *
	 * 而除了每个 DispatcherServlet 配置类的作用上下文之外，还有一个根应用上下文，这个应用上下文的作用是为了在多个 DispatcherServlet
	 * 之间共享 Bean，这就是 getRootConfigClasses 的作用，用户返回根应用上下文的配置类。Sprig 框架的机制会保证如果在当前 DispatcherServlet
	 * 的应用上下文中没有找到想要的 bean 时，会去根应用上下文中去找。
	 *
