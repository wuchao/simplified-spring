...
public class DispatcherServlet extends FrameworkServlet {
	...
	private void initViewResolvers(ApplicationContext context) {
		...
		if (this.detectAllViewResolvers) {
			// 查找所有 ViewResolver 实例
			// BeanNameViewResolver、ViewResolverComposite、InternalResourceViewResolver、ContentNegotiatingViewResolver
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				 ...
				 */
				// 最终会调用 HandlerInterceptor 的 preHandle 方法
				if (!mappedHandler.applyPreHandle(processedRequest, response)) {
					return;
				}

				// 调用具体的 Controller 中的接口方法
				...
				// 最终会调用 HandlerInterceptor 的 postHandle 方法
