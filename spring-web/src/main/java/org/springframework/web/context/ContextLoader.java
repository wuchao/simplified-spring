...
public class ContextLoader {
    ...
    protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
        // 决定到底用哪个 WebApplicationContext 的类，首先从 web.xml 中 <context-param> 的 <param-name> 中找 contextClass，
        // 看是否有指定某个 class，如果有指定，则载入对应的 class，如果没有指定，就默认用 XmlWebApplicationContext
        Class<?> contextClass = determineContextClass(sc);
        // 判断是否为 ConfigurableWebApplicationContext.class 或从它继承
    ...
    protected Class<?> determineContextClass(ServletContext servletContext) {
        // 从 servletContext 中读取 contextClassName
        String contextClassName = servletContext.getInitParameter(CONTEXT_CLASS_PARAM);
        if (contextClassName != null) {
            try {
                // 如果从 servletContext 读取到的 contextClassName 不为空，就返回对应的 class
            ...
            // 如果从 servletContext 中读取到的 contextClassName 为空就取默认的 className
            contextClassName = defaultStrategies.getProperty(WebApplicationContext.class.getName());
            try {
                // 返回 className 对应的 class
    ...
    protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
        // if 里的条件相等，表示 wac 的 id 的值还是类初始化时的默认值
        ...
        // 把 servletContext 放到 webApplicationContext 中，以后可以直接取出来用
        ...
        if (env instanceof ConfigurableWebEnvironment) {
            // wac 的 refresh 方法里调用 prepareRefresh 方法，里面还会再调用该方法
